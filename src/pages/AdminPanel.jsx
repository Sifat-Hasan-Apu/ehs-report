                        ...
                        {activeSection === 6 && (
                            <div className="space-y-8 animate-in slide-in-from-bottom-2 duration-500">
                                <div>
                                    ...
                                    {/* Initialize default permits if empty */}
                                    {(() => {
                                        if (!formData.permits || formData.permits.length === 0) {
                                            ...
                                            // We can't setState inside render/return easily, but we can render the initial state controls
                                            // Better to use a button or effect, but for now we'll check length
                                            // Actually, let's just provide a button to load defaults if empty, 
                                            // or handle it in the .map if we want to be safe. 
                                            // But for best UX, let's assume if it's empty we show a button or just the inputs for defaults.
                                            // To keep it simple, if no permits, we can just render the inputs for the defaults and update state on change.
                                        }
                                        return null;
                                    })()}
                                    ...
                                    <div className="border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                                        <table className="w-full text-sm text-left">
                                            ...
                                            <tbody className="divide-y divide-slate-100">
                                                {(formData.permits?.length > 0 ? formData.permits : [
                                                    { type: "Work at Height", opened: 0, closed: 0, violations: 0 },
                                                    { type: "Hot Work", opened: 0, closed: 0, violations: 0 },
                                                    { type: "Confined Space", opened: 0, closed: 0, violations: 0 },
                                                    { type: "Lifting", opened: 0, closed: 0, violations: 0 },
                                                    ...
                                                    <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                                        ...
                                                        <td className="px-6 py-3">
                                                            <input
                                                                ...
                                                                value={permit.opened}
                                                                onChange={(e) => {
                                                                    const newPermits = [...(formData.permits || [
                                                                        { type: "Work at Height", opened: 0, closed: 0, violations: 0 },
                                                                        { type: "Hot Work", opened: 0, closed: 0, violations: 0 },
                                                                        { type: "Confined Space", opened: 0, closed: 0, violations: 0 },
                                                                        { type: "Lifting", opened: 0, closed: 0, violations: 0 },
                                                        ...
                                                        <td className="px-6 py-3">
                                                            <input
                                                                ...
                                                                className="w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-center font-bold text-emerald-600 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none"
                                                            />
                                                        </td>
                                                        <td className="px-6 py-3">
                                                            <input
                                                                ...
                                                                className={`w-full px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-center font-bold focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none ${permit.violations > 0 ? 'text-red-500 bg-red-50 border-red-200' : 'text-slate-400'}`}
                                                            />
                                                        </td>
                                                    </tr>
                                                ))}
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea
                                        ...
                                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition-all h-24"
                                        placeholder="e.g. Critical lift of Gas Turbine auxiliary skid completed successfully."
                                    />
                                    <p className="text-xs text-slate-400">Mention any significant achievements or notes.</p>
                                </div>
                            </div>
                        )}
